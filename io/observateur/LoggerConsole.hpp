...
#include "../evenement/EvenementIteration.hpp"
#include "../evenement/EvenementTerminaision.hpp"
...
public:
	...
	void onNotify(const Evenement<N>& event) override 
	{
		...
		if (auto* e = dynamic_cast<const EvenementDemarrage<N>*>(&event)) {
			std::cout << std::setw(3) << "" << "Point initial : "; e->x_initial.print();
			std::cout << std::endl;
		...
		else if (auto* e = dynamic_cast<const EvenementIteration<N>*>(&event)) {
		...
		else if (auto* e = dynamic_cast<const EvenementTerminaison<N>*>(&event)){
			std::cout << std::endl;
			std::cout << std::setw(3) << "" << "Solution : " << std::endl;
			std::cout << std::setw(5) << ""<< " Statut : " << e->status << std::endl;
			std::cout << std::setw(5) << ""<< " Solution : "; e->solution_finale.print();
			std::cout << std::endl;
